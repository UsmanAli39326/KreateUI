...
export function groupScansByWeek(scans = []) {
  ...
  const earliest = new Date(sorted[0].createdAt || sorted[0].date);
  ...
  sorted.forEach((scan) => {
    const date = new Date(scan.createdAt || scan.date);
    ...
    weekMap[weekKey].totalScore += (scan.score || scan.summary?.currentScore || 0);
  });

  const finalArray = Object.values(weekMap).sort((a, b) =>
    parseInt(a.week.slice(1)) - parseInt(b.week.slice(1))
  );

  finalArray.forEach(w => {
    w.averageScore = w.count > 0 ? Math.round(w.totalScore / w.count) : 0;
  });
...
/**
 * Calculates a simple week-over-week trend percentage based on average score.
 * Compares the last two weeks. Returns 0 if not enough data.
 *
 * @param {Array<{ averageScore: number }>} weeklyData
 * @returns {number} Percentage change (positive = up, negative = down)
 */
export function calculateTrend(weeklyData = []) {
  if (weeklyData.length < 2) return 0;
  const last = weeklyData[weeklyData.length - 1].averageScore;
  const prev = weeklyData[weeklyData.length - 2].averageScore;
